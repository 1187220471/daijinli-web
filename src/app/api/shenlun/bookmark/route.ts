...
export async function POST(request: Request) {
  try {
    ...
    const bookmark = await prisma.shenlunBookmark.upsert({
    ...
    console.error('Shenlun bookmark error:', error)
...
export async function DELETE(request: Request) {
  try {
    ...
    await prisma.shenlunBookmark.delete({
      where: { userId_questionId: { userId: auth.userId, questionId } },
    })

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Shenlun bookmark delete error:', error)
