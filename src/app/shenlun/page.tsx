...
export default function ShenlunPage() {
  ...
  const [questions, setQuestions] = useState<ShenlunItem[]>([])
  ...
  const fetchQuestions = useCallback(async (p = 1) => {
    ...
    try {
      const res = await fetch(`/api/shenlun/list?${params}`, {
  ...
  const grouped = useMemo(() => {
    return displayedQuestions.reduce<Record<string, ShenlunItem[]>>((acc, q) => {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <h1 className="text-lg font-bold text-slate-800">📝 申论真题参考</h1>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            >
              ...
              {filters.categories.map((c) => (
                <option key={c} value={c}>{c}卷</option>
              ))}
            </select>
                ...
                <div className="divide-y divide-slate-100">
                  {qs
                    ...
                    .map((q) => (
                      <button
                        key={q.id}
                        onClick={() => router.push(`/shenlun/${q.id}`)}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-2">
                              <span className="text-xs text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">{q.questionType}</span>
                              {q.score && <span className="text-xs text-slate-400">{q.score}分</span>}
                              {q.wordLimit && <span className="text-xs text-slate-400">{q.wordLimit}</span>}
                              {q.materialRange && <span className="text-xs text-slate-400">{q.materialRange}</span>}
                              <span className="text-xs text-slate-400">{q.answerCount} 版答案</span>
                              {q.isBookmarked && (
                                <>
